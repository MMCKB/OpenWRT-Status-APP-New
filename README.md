...
- **多设备**：「设备」页以卡片管理多台路由器：点卡片切换当前设备，向左滑动卡片呼出
  编辑 / 删除（删除有二次确认）。旧版单机配置会自动迁移；连接与 SSH 配置在设备
  添加/编辑页填写。
- **系统资源**：1/5/15 分钟平均负载、内存与交换分区占用。
- **网络流量**：各网络接口（`lan` / `wan` / `wlan`）的累计收发字节与实时速率（↓/↑）。
- **监控**：流量与资源折线图（最近 60 个采样点），以及网络接口（状态 / IPv4 / 运行时长 /
  收发计数）、无线（SSID / 信道 / 客户端数）、DHCP 租约（经 SSH 读取 `/tmp/dhcp.leases`），
  均作用于当前设备。
- **SSH 远程终端**：持久 PTY Shell 会话，支持交互式命令与常用快捷命令。
- **自动刷新**：按固定间隔（默认 5 秒）自动轮询；支持手动刷新。
- **本地保存**：设备列表与 SSH 配置保存在本机 `SharedPreferences`。
...
> 且为自签名证书，请在设备编辑页开启「忽略证书校验」。账号密码仅保存在本机，不会上传。
